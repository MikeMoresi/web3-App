...
@pytest.fixture
def testBasil(eth_tester, w3):

    deploy_address = eth_tester.get_accounts()[0]

    # Create our contract class.
    testBasil = w3.eth.contract(address = contractAddress,abi=abi )
    # issue a transaction to deploy the contract.
    tx_hash = testBasil.constructor().transact({
        'from': deploy_address,
    })
    # wait for the transaction to be mined
    tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash, 180)
    # instantiate and return an instance of our contract.
    return testBasil(tx_receipt.contractAddress)


def test_initial_greeting(testBasil):
    hw = testBasil.caller.bar()
    assert hw == "hello world"


def test_can_update_greeting(w3, testBasil):
    # send transaction that updates the greeting
    tx_hash = testBasil.functions.setBar(
...
def test_updating_greeting_emits_event(w3, testBasil):
    # send transaction that updates the greeting
    tx_hash = testBasil.functions.setBar(
        "testing contracts is easy",
    ).transact({
    ...
    logs = testBasil.events.barred.getLogs()
    assert len(logs) == 1
